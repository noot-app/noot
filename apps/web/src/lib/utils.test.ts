...
describe('getValidatedSession', () => {
  ...
  it('returns a normalized session when claims are valid', async () => {
    ...
    mockSupabase.auth.getSession.mockResolvedValue({ data: { session: rawSession } });
    mockSupabase.auth.getClaims.mockResolvedValue({
      data: { claims: { exp: Math.round(Date.now() / 1000) + 3600, sub: 'uid', email: 'e@example.com', phone: null, user_metadata: {}, app_metadata: {}, is_anonymous: false } },
      error: null
    });
  ...
  it('returns null when getClaims returns error', async () => {
    mockSupabase.auth.getSession.mockResolvedValue({ data: { session: { access_token: 'token', refresh_token: 'r' } } });
    mockSupabase.auth.getClaims.mockResolvedValue({ data: null, error: { message: 'bad' } });
    const res = await getValidatedSession(mockSupabase);
    expect(res).toBeNull();
  });
