...
// Uses double-submit cookie pattern with JWT-based authentication
func CSRFMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if method == "POST" || method == "PUT" || method == "DELETE" || method == "PATCH" {
			// Generate CSRF token if not exists or if it's a GET request setting up the page
			csrfToken := c.GetHeader("X-CSRF-Token")
			
			// For non-GET requests, validate the token
			if method != "GET" {
				if csrfToken == "" {
					LogWarn("CSRF token missing", "method", method, "path", c.Request.URL.Path)
				...
				// Basic CSRF token validation (ensure it's a valid hex string of expected length)
				if len(csrfToken) != 64 { // 32 bytes = 64 hex chars
					LogWarn("Invalid CSRF token format", "method", method, "path", c.Request.URL.Path)
					c.JSON(http.StatusForbidden, gin.H{
				...
				if _, err := hex.DecodeString(csrfToken); err != nil {
					LogWarn("CSRF token not valid hex", "method", method, "path", c.Request.URL.Path)
					c.JSON(http.StatusForbidden, gin.H{
		...
		// For GET requests, provide a CSRF token in response header for subsequent requests
		if method == "GET" {
			token, err := generateCSRFToken()
			if err != nil {
				LogError("Failed to generate CSRF token", err)
