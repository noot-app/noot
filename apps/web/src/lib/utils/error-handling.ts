 ...
 */
type ErrorLike = string | Error | { message?: string; error?: string } | unknown
...
export function formatErrorForUser(
  ...
  const defaultTranslations: Record<string, string> = {
    "At least one override must be provided":
      "At least one nutrition target override must be provided",
  ...
  return dev
    ? `${errorMessage} (Raw: ${JSON.stringify(rawError)})`
    : errorMessage
}
