<script lang="ts">
  export let variant: 'primary' | 'secondary' | 'accent' | 'error' | 'warning' | 'info' | 'success' | 'ghost' = 'primary';
  ...
  $: classes = [baseClasses, variantClass, sizeClass, modifierClasses, className]
    .filter(Boolean)
    .join(' ');
</script>
