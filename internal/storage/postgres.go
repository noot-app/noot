...
func (s *PostgreSQLStore) CreateConsumptionItem(ctx context.Context, item *ConsumptionItem) error {
	...
	query := `
		INSERT INTO consumption_items (
			id, consumption_id, item_id, name, brand, grams, user_quantity, user_unit, label, note,
			calories, protein_g, total_fat_g, saturated_fat_g, trans_fat_g, cholesterol_mg, sodium_mg,
			total_carbs_g, dietary_fiber_g, total_sugars_g, added_sugars_g, vitamin_a_mcg, vitamin_c_mg,
			vitamin_d_mcg, vitamin_e_mg, vitamin_k_mcg, thiamine_mg, riboflavin_mg, niacin_mg, vitamin_b6_mg,
			folate_mcg, vitamin_b12_mcg, biotin_mcg, pantothenic_acid_mg, choline_mg, calcium_mg, iron_mg,
			magnesium_mg, phosphorus_mg, potassium_mg, zinc_mg, copper_mg, manganese_mg, selenium_mcg,
			iodine_mcg, molybdenum_mcg, chromium_mcg, fluoride_mg, chloride_mg, omega3_ala_g, omega3_epa_g,
			omega3_dha_g, omega6_g, creatine_mg, caffeine_mg, alcohol_g, polyunsaturated_fat_g,
			monounsaturated_fat_g, created_at
		) VALUES (
	...
	_, err := s.db.ExecContext(ctx, query,
		item.ID, item.ConsumptionID, item.ItemID, item.Name, item.Brand, item.Grams,
		item.UserQuantity, item.UserUnit, item.Label, item.Note,
		...
		item.CalciumMg, item.IronMg, item.MagnesiumMg, item.PhosphorusMg, item.PotassiumMg,
		item.ZincMg, item.CopperMg, item.ManganeseMg, item.SeleniumMcg, item.IodineMcg,
		item.MolybdenumMcg, item.ChromiumMcg, item.FluorideMg, item.ChlorideMg, item.Omega3AlaG,
		item.Omega3EpaG, item.Omega3DhaG, item.Omega6G, item.CreatineMg, item.CaffeineMg,
		item.AlcoholG, item.PolyunsaturatedFatG, item.MonounsaturatedFatG, item.CreatedAt)
...
func (s *PostgreSQLStore) GetConsumptionItems(ctx context.Context, consumptionID string) ([]*ConsumptionItem, error) {
	query := `
		SELECT id, consumption_id, item_id, name, brand, grams, user_quantity, user_unit, label, note,
		       calories, protein_g, total_fat_g, saturated_fat_g, trans_fat_g, cholesterol_mg, sodium_mg,
		       total_carbs_g, dietary_fiber_g, total_sugars_g, added_sugars_g, vitamin_a_mcg, vitamin_c_mg,
		       vitamin_d_mcg, vitamin_e_mg, vitamin_k_mcg, thiamine_mg, riboflavin_mg, niacin_mg, vitamin_b6_mg,
		       folate_mcg, vitamin_b12_mcg, biotin_mcg, pantothenic_acid_mg, choline_mg, calcium_mg, iron_mg,
		       ...
		       iodine_mcg, molybdenum_mcg, chromium_mcg, fluoride_mg, chloride_mg, omega3_ala_g, omega3_epa_g,
		       omega3_dha_g, omega6_g, creatine_mg, caffeine_mg, alcohol_g, polyunsaturated_fat_g,
		       monounsaturated_fat_g, created_at, updated_at
	...
	for rows.Next() {
		...
		err := rows.Scan(&item.ID, &item.ConsumptionID, &item.ItemID, &item.Name, &item.Brand,
			...
			&item.MolybdenumMcg, &item.ChromiumMcg, &item.FluorideMg, &item.ChlorideMg, &item.Omega3AlaG,
			&item.Omega3EpaG, &item.Omega3DhaG, &item.Omega6G, &item.CreatineMg, &item.CaffeineMg,
			&item.AlcoholG, &item.PolyunsaturatedFatG, &item.MonounsaturatedFatG, &item.CreatedAt, &item.UpdatedAt)
		if err != nil {
